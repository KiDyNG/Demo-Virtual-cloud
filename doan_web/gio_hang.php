                                    ...
                                    <tr>
                                        ...
                                        <td> 
                                            <?php
                                                ...
                                                $sql = "SELECT size FROM sanpham WHERE sp_id =  '".$val['id']."'";
                                                $result = mysqli_query($ket_noi, $sql);
                                                echo "<select name='size' size='1'>";
                                                if (mysqli_num_rows($result) > 0) {
                                                    // vòng lặp để tạo các lựa chọn
                                                    while($row = mysqli_fetch_assoc($result)) {
                                                        $sizes = explode(',', $row['size']); // tách các kích thước ra thành mảng
                                                        foreach ($sizes as $size) {
                                                            echo "<option id='size' value='".$size."'>".$size."</option>";
                                                        }
                                                    }
                                                }
                                                echo "</select>";
